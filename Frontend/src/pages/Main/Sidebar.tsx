...
export default function Sidebar({ menuItems, selectedItem, onSelectItem }: Props) {
  ...
  const [openLogout, setOOpenLogout] = useState(false)

  const handleLogout = () => {
    console.log(openLogout)

    setOOpenLogout((prev) => !prev)
  }
  ...
  return (
    <div className=' w-full px-4 pt-4  mx-auto  relative'>
      ...
      <div className='  my-2'>
        <Popover initalOpen={openLogout} className='flex items-center   hover:text-white/70 cursor-pointer  w-full mx-auto ' renderPopover={<Logout />}>
          <div
